...
class GlobalVar(object):
    ...
    @classmethod
    def create_private_room(cls) -> Room:
        """创建私人房间（不添加机器人）"""
        room = Room(cls.gen_room_id(), level=1, allow_robot=False)
        cls.__waiting_rooms__[room.room_id] = room
        logging.info('PRIVATE ROOM[%d] CREATED', room.room_id)
        return room

    @classmethod
    def find_room_by_id(cls, room_id: int):
        """按房间号精确查找，找不到返回 None"""
    ...
    def gen_room_id(cls) -> int:
        ...
        for _ in range(100):
            ...
            if room_id not in cls.__waiting_rooms__ and room_id not in cls.__playing_rooms__:
                return room_id
        cls.total_room_count += 1
        return cls.total_room_count + 10000
