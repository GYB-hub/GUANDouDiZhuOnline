...
class Player(object):
    ...
    def handle_init(self, code: int, packet: Dict[str, Any]):
        from .globalvar import GlobalVar
        if code == Pt.REQ_NEW_ROOM:
            # 创建私人房间并加入
            room = GlobalVar.create_private_room()
            self.state = State.WAITING
            if self.join_room(room):
                self.write_message([Pt.RSP_NEW_ROOM, {'room': room.room_id}])
                self.room.sync_room()
            logger.info('PLAYER[%s] CREATED ROOM[%d]', self.uid, room.room_id)
        elif code == Pt.REQ_JOIN_ROOM:
            room_id, level = packet.get('room', -1), packet.get('level', 1)
            if room_id > 0:
                # 按房间号加入指定房间
                room = GlobalVar.find_room_by_id(room_id)
                if not room:
                    self.write_error('房间 %s 不存在' % room_id)
                    return
            else:
                # 快速匹配
                room = GlobalVar.find_room(room_id, level, self.allow_robot)
