...
export class Game {
    ...
    init(options) {
        if (typeof options === 'object') {
            this.gameMode = options.mode || 'ai';
            this.targetRoom = options.room || -1;
        } else {
            this.gameMode = 'ai';
            this.targetRoom = -1;
        }
    ...
    create() {
        ...
        const titleBar = this.game.add.text(width / 2, 0, '', {
            font: "22px",
            fill: "#fff",
            align: "center"
        });
        ...
        // 房间号醒目提示（好友房模式）
        const roomCodeText = this.game.add.text(width / 2, 50, '', {
            font: "bold 30px Arial",
            fill: "#FFD700",
            align: "center"
        });
        roomCodeText.anchor.set(0.5, 0);

        // 等待好友提示
        const waitingText = this.game.add.text(width / 2, height * 0.42, '', {
            font: "22px Arial",
            fill: "#cccccc",
            align: "center"
        });
        ...
        observer.subscribe('room', function (room) {
            ...
            if (gameMode !== 'ai') {
                roomCodeText.text = `房间号: ${room.id}  (告诉好友输入此号码加入)`;
            }
        });
        observer.subscribe('playerCount', function (count) {
            if (gameMode !== 'ai') {
                waitingText.text = count < 3 ? `等待好友加入中... (${count}/3)` : '';
                if (count >= 3) {
                    roomCodeText.text = '';
                }
            }
    ...
    onopen() {
        ...
        if (this.gameMode === 'create') {
            this.socket.send([Protocol.REQ_NEW_ROOM, {}]);
        } else if (this.gameMode === 'join' && this.targetRoom > 0) {
            this.socket.send([Protocol.REQ_JOIN_ROOM, {"room": this.targetRoom, "level": 1}]);
        } else {
            // 人机模式，快速匹配
            this.socket.send([Protocol.REQ_JOIN_ROOM, {"room": -1, "level": 1}]);
        }
    }
    ...
    onmessage(message) {
        ...
        switch (code) {
            case Protocol.ERROR:
                console.error('Server Error:', packet['reason']);
                if (!observer.get('room')) {
                    alert(packet['reason']);
                    this.quitGame();
                }
                break;
            case Protocol.RSP_ROOM_LIST:
                ...
                break;
            case Protocol.RSP_NEW_ROOM:
                // 房间已在服务端创建并加入，等待 RSP_JOIN_ROOM
                console.log('Room created:', packet['room']);
                break;
            case Protocol.RSP_JOIN_ROOM: {
                ...
                for (let i = 0; i < syncInfo.length; i++) {
                    if (syncInfo[i].uid) playerCount++;
                }
                observer.set('playerCount', playerCount);
                for (let i = 0; i < syncInfo.length; i++) {
